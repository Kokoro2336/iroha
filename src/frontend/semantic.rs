...
impl Semantic {
    ...
    pub fn analyze(&mut self, node_id: NodeId) -> Result<Type, String> {
        ...
        match node_type {
            NodeType::BinaryOp => {
                let (mut lhs_id, mut rhs_id, op_kind) = match &self.ast[node_id] {
                ...
                if matches!(op_kind, Op::And | Op::Or) {
                    if matches!(lhs_type, Type::Float) {
                        ...
                        lhs_id = self.ast.alloc(Node::BinaryOp {
                            typ: Type::Int,
                            lhs: lhs_id,
                            op: Op::Ne,
                            rhs: zero_id,
                    ...
                    if matches!(rhs_type, Type::Float) {
                        let zero_id = self.ast.alloc(Node::Literal(Literal::Float(0.0)));
                        rhs_id = self.ast.alloc(Node::BinaryOp {
                            typ: Type::Int,
                            lhs: rhs_id,
                            op: Op::Ne,
                            rhs: zero_id,
                ...
                if matches!(lhs_type, Type::Int) && matches!(rhs_type, Type::Float) {
                    lhs_id = self.ast.alloc(Node::UnaryOp {
                        typ: Type::Float,
                ...
                if let Node::BinaryOp { typ, lhs, rhs, .. } = &mut self.ast[node_id] {
                    *typ = ret_typ.clone();
                    *lhs = lhs_id;
                    *rhs = rhs_id;
                }
            ...
            NodeType::UnaryOp => {
                let (op_kind, mut operand_id) = match &self.ast[node_id] {
                    Node::UnaryOp { op, operand, .. } => (op.clone(), *operand),
                    _ => unreachable!(),
                };
                ...
                match op_kind {
                    Op::Plus => {
                        self.ast[node_id] = self.ast[operand_id].clone();
                        Ok(operand_type)
                    }
                    Op::Minus => {
                        if let Node::UnaryOp { typ, .. } = &mut self.ast[node_id] {
                    ...
                    Op::Not => {
                        if matches!(operand_type, Type::Float) {
                            let zero_id = self.ast.alloc(Node::Literal(Literal::Float(0.0)));
                            operand_id = self.ast.alloc(Node::BinaryOp {
                                typ: Type::Int,
                                lhs: operand_id,
                                op: Op::Ne,
                                rhs: zero_id,
                    ...
                    _ => unreachable!("Unexpected unary op: {:?}", op_kind),
                }
            }
