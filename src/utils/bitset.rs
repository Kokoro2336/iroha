...
mod tests {
    ...
    fn test_insert_contains() {
        ...
        assert!(bs.insert(0));
        assert!(bs.contains(0));
        assert!(!bs.insert(0)); // Already present
    ...
    #[test]
    fn test_ops() {
        let bs1 = bitset![1, 2, 3];
        let bs2 = bitset![3, 4, 5];

        let union_bs = &bs1 | &bs2;
        assert!(union_bs.contains(1));
        assert!(union_bs.contains(3));
        assert!(union_bs.contains(5));

        let intersection_bs = &bs1 & &bs2;
        assert!(!intersection_bs.contains(1));
        assert!(intersection_bs.contains(3));
        assert!(!intersection_bs.contains(5));

        let diff_bs = &bs1 - &bs2;
        assert!(diff_bs.contains(1));
        assert!(!diff_bs.contains(3));
        assert!(!diff_bs.contains(4));
    }

    #[test]
    fn test_iter() {
        let bs = bitset![1, 10, 65];
        let vec: Vec<usize> = bs.iter().collect();
        assert_eq!(vec, vec![1, 10, 65]);
    }
